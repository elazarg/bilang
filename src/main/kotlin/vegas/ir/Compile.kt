...
private fun lowerQuery(query: Query, kind: Kind, typeEnv: Map<AstType.TypeId, AstType>): Signature {
    return Signature(
        join = (kind == Kind.JOIN || kind == Kind.JOIN_CHANCE),
...
private fun lowerExpr(exp: AstExpr, typeEnv: Map<AstType.TypeId, AstType>): Expr {
    return when (exp) {
        // Literals
        is AstExpr.Const.Num -> Expr.IntLit(exp.n)
        is AstExpr.Const.Bool -> Expr.BoolLit(exp.truth)
        is AstExpr.Const.Address -> Expr.IntLit(exp.n)
        ...
        is AstExpr.Call -> when (exp.target.id.name) {
            "alldiff" -> {
                ...
                if (args.size < 2) {
                    Expr.BoolLit(true) // alldiff of 0 or 1 elements is trivially true
                } else {
    ...
    return when (outcome) {
        ...
        is Outcome.Cond -> {
            ...
            allRoles.associateWith { role ->
                val t = ifTrue[role] ?: Expr.IntLit(0) // Default to 0 if role not in branch
                val f = ifFalse[role] ?: Expr.IntLit(0)
                Expr.Ite(cond, t, f)
            }
        }
