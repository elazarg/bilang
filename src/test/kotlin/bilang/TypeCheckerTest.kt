...
private object B {
    ...
    fun m(role: String, field: String) = Member(role, field)

    fun not(e: Exp) = UnOp("!", e)
    fun isDef(e: Exp) = UnOp("isDefined", e)
    fun isUndef(e: Exp) = UnOp("isUndefined", e)
    fun abs(e: Exp) = Exp.Call(Var("abs"), listOf(e))

    fun ite(c: Exp, t: Exp, f: Exp) = Exp.Cond(c, t, f)

    // let (expression & outcome)
    fun letE(name: String, t: TypeExp, init: Exp, body: Exp) =
        Exp.Let(name to t, init, body)

    fun letO(name: String, t: TypeExp, init: Exp, body: Outcome) =
        Outcome.Let(name to t, init, body)

    // param declarations
    fun i(name: String) = name to INT
    fun bl(name: String) = name to BOOL
    fun opt(t: TypeExp) = TypeExp.Opt(t)
    fun hidden(t: TypeExp) = TypeExp.Hidden(t)

    // queries and binds
    private fun q(role: String, params: List<VarDec> = emptyList(), deposit: Int = 0, where: Exp = b(true)) =
        Query(role = role, params = params, deposit = n(deposit), where = where)

    fun join(role: String, params: List<VarDec> = emptyList(), deposit: Int = 0, where: Exp = b(true)) =
        Ext.Bind(Kind.JOIN, listOf(q(role, params, deposit, where)), Ext.Value(Value(emptyMap())))

    fun yieldTo(role: String, params: List<VarDec>, where: Exp = b(true)) =
        Ext.Bind(Kind.YIELD, listOf(q(role, params, 0, where)), Ext.Value(Value(emptyMap())))

    fun reveal(role: String, params: List<VarDec>, where: Exp = b(true)) =
        Ext.Bind(Kind.REVEAL, listOf(q(role, params, 0, where)), Ext.Value(Value(emptyMap())))

    fun joinChance(role: String, where: Exp = b(true)) =
        Ext.Bind(Kind.JOIN_CHANCE, listOf(q(role, emptyList(), 0, where)), Ext.Value(Value(emptyMap())))

    // payouts
    fun pay(vararg outs: Pair<String, Exp>) = Value(mapOf(*outs))
...
class TypeCheckerTest : FreeSpec({
    ...
    "Type System - Custom Types" - {
        ...
        "should handle finite set types correctly" - {
            withData(
                ...
                B.program(
                    types = mapOf("door" to TypeExp.Subset(setOf(B.n(0), B.n(1), B.n(2)))),
                    B.join("P"),
                    B.yieldTo("P", listOf("d" to TypeExp.TypeId("door")))
                ),
                // type coin = {0,1}; join P(); yield P(flip: coin)
                B.program(
                    types = mapOf("coin" to TypeExp.Subset(setOf(B.n(0), B.n(1)))),
                    B.join("P"),
                    B.yieldTo("P", listOf("flip" to TypeExp.TypeId("coin")))
                ),
                // type small, big sets; yield both
                B.program(
                    types = mapOf(
                        "small" to TypeExp.Subset(setOf(B.n(1), B.n(2), B.n(3))),
                        "big" to TypeExp.Subset(setOf(B.n(10), B.n(20), B.n(30)))
                    ),
                    B.join("P"),
                    B.yieldTo("P", listOf("s" to TypeExp.TypeId("small"), "b" to TypeExp.TypeId("big")))
                )
            ) { prog ->
        ...
        "should handle range types correctly" - {
            withData(
                B.program(
                    types = mapOf("digit" to TypeExp.Range(B.n(0), B.n(9))),
                    B.join("P"),
                    B.yieldTo("P", listOf("d" to TypeExp.TypeId("digit")))
                ),
                B.program(
                    types = mapOf("percent" to TypeExp.Range(B.n(0), B.n(100))),
                    B.join("P"),
                    B.yieldTo("P", listOf("p" to TypeExp.TypeId("percent")))
                )
            ) { prog ->
                shouldNotThrow<StaticError> { typeCheck(prog) }
            }
            ...
            "accepts valid where constraints" {
                val validCases = listOf(
                    B.program(
                        ...
                        B.yieldTo(
                            "P",
                            listOf("d" to TypeExp.TypeId("door")),
                    ...
                    B.program(
                        ...
                        B.yieldTo(
                            "P",
                            listOf("c" to TypeExp.TypeId("coin")),
                            where = (B.m("P", "c") eq B.n(0)) or (B.m("P", "c") eq B.n(1))
                        )
                    ),
                    B.program(
                        types = mapOf("range" to TypeExp.Range(B.n(1), B.n(10))),
                        B.join("P"),
                        B.yieldTo(
                            "P",
                            listOf("r" to TypeExp.TypeId("range")),
    ...
    "Type System - Hidden Types" - {
        ...
        "should handle hidden type declarations" - {
            withData(
                ...
                B.program(
                    B.join("H"),
                    B.yieldTo("H", listOf("secret" to B.hidden(INT)))
                ),
                // type door = {0,1,2}; join H(); yield H(car: hidden door)
                B.program(
                    types = mapOf("door" to TypeExp.Subset(setOf(B.n(0), B.n(1), B.n(2)))),
                    B.join("H"),
                    B.yieldTo("H", listOf("car" to B.hidden(TypeExp.TypeId("door"))))
                ),
                // join H(); yield H(x: hidden int, y: int)
                B.program(
                    B.join("H"),
                    B.yieldTo("H", listOf("x" to B.hidden(INT), "y" to INT))
                ),
                // join H(); yield H(a: hidden bool, b: hidden bool)
                B.program(
                    B.join("H"),
                    B.yieldTo("H", listOf("a" to B.hidden(BOOL), "b" to B.hidden(BOOL)))
                )
            ) { prog ->
                shouldNotThrow<StaticError> { typeCheck(prog) }
            }
        ...
        "should validate reveal operations" - {
            ...
            "accepts revealing hidden values" {
                val validReveals = listOf(
                    ...
                    B.program(
                        B.join("H"),
                        B.yieldTo("H", listOf("s" to B.hidden(INT))),
                        B.reveal("H", listOf("s" to INT))
                    ),
                    ...
                    B.program(
                        ...
                        B.join("H"),
                        B.yieldTo("H", listOf("car" to B.hidden(TypeExp.TypeId("door")))),
                        B.reveal("H", listOf("car" to TypeExp.TypeId("door")))
                    ),
                    // join H(); yield H(a: hidden int, b: hidden bool); reveal H(a: int, b: bool)
                    B.program(
                        B.join("H"),
                        B.yieldTo("H", listOf("a" to B.hidden(INT), "b" to B.hidden(BOOL))),
                        B.reveal("H", listOf("a" to INT, "b" to BOOL))
                    )
                )
            ...
            "rejects revealing non-hidden values" {
                val bad = B.program(
                    B.join("H"),
                    B.yieldTo("H", listOf("public" to INT)),
                    B.reveal("H", listOf("public" to INT))
                )
                val ex = shouldThrow<StaticError> { typeCheck(bad) }
            ...
            "rejects type mismatches in reveal" {
                val bad = B.program(
                    B.join("H"),
                    B.yieldTo("H", listOf("s" to B.hidden(INT))),
                    B.reveal("H", listOf("s" to BOOL)) // mismatch: int vs bool
                )
                val ex = shouldThrow<StaticError> { typeCheck(bad) }
        ...
        "should validate role declarations" - {
            ...
            "accepts valid role usage" {
                val validRoles = listOf(
                    ...
                    B.program(B.join("Player1", deposit = 100), B.join("Player2", deposit = 200)),
                    B.program(B.join("A"), B.join("B"), B.join("C"), B.join("D"))
                )
            ...
            "rejects accessing undefined roles" {
                val bad = B.program(
                    B.yieldTo("UnknownRole", listOf(B.i("x")))
                )
                val ex = shouldThrow<StaticError> { typeCheck(bad) }
                ex.message shouldContain "is undefined"
            }
    ...
    "Type System - Operators" - {
        ...
        "null checking operators" - {
            ...
            "isDefined and isUndefined" {
                // emulate “!= null” with isDefined / isUndefined
                val p1 = B.program(
                    B.join("P"), B.join("Q"),
                    B.yieldTo("P", listOf(B.i("x"))),
                    value = Value(
                        mapOf(
                            "P" to B.ite(B.isDef(B.m("P", "x")), B.n(100), B.n(0)),
                ...
                val p2 = B.program(
                    B.join("P"),
                    B.yieldTo("P", listOf("opt" to B.opt(INT))),
                    value = B.pay(
                        "P" to B.ite(B.isUndef(B.m("P", "opt")), B.n(-100), B.m("P", "opt"))
                    )
                )
                shouldNotThrow<StaticError> { typeCheck(p2) }
            }
        ...
        "should type check ternary conditionals" - {
            ...
            "branches must have compatible types" {
                ...
                val p = B.program(
                    types,
                    B.join("P"),
                    B.yieldTo("P", listOf(B.bl("b"), "s" to TypeExp.TypeId("small"), "g" to TypeExp.TypeId("big"))),
            ...
            val p = B.program(
                ...
                value = B.letO(
                    ...
                    Value(
                        mapOf(
                            "P" to B.v("total"),
                            "Q" to Exp.BinOp("-", B.n(0), B.v("total"))
                        )
                    )
    ...
    "Type System - Function Calls" - {
        ...
        "should type check built-in functions" {
            ...
            val p = B.program(
                ...
                B.yieldTo("P", listOf(B.i("x"))),
                value = B.pay(
                    "P" to Exp.Call(B.v("abs"), listOf(B.m("P", "x") minus B.n(10)))
                )
            )
            shouldNotThrow<StaticError> { typeCheck(p) }
        }
        ...
        "should reject unknown functions" {
            val bad = B.program(
                B.join("P"),
                value = B.pay(
                    "P" to Exp.Call(B.v("unknown_function"), listOf(B.n(5)))
                )
            )
    ...
    "Built-ins - alldiff" - {
        ...
        "accepts alldiff over integers (constants and members)" {
            ...
            val pConst = B.program(
                ...
                value = B.pay(
                    "P" to B.ite(
                        Exp.Call(B.v("alldiff"), listOf(B.n(1), B.n(2), B.n(3))),
                        B.n(100), B.n(0)
                    )
                )
            )
        ...
        "rejects alldiff with mixed or non-integer arguments" {
            ...
            val badMixed = B.program(
                ...
                value = B.pay(
                    "P" to B.ite(
                        Exp.Call(B.v("alldiff"), listOf(B.m("P", "x"), B.n(5), B.m("P", "b"))),
                        B.n(1), B.n(0)
                    )
                )
            )
            ...
            val badBool = B.program(
                ...
                value = B.pay(
                    "P" to B.ite(
                        Exp.Call(B.v("alldiff"), listOf(B.m("P", "p"), B.m("P", "q"))),
                        B.n(1), B.n(0)
                    )
                )
            )
    ...
    "Type System - Complex Game Scenarios" - {
        ...
        "Monty Hall game type checking" {
            val types = mapOf(
                "door" to TypeExp.Subset(setOf(B.n(0), B.n(1), B.n(2)))
            )
            val p = B.program(
                types,
                B.join("Host", deposit = 100),
                B.join("Guest", deposit = 100),
                B.yieldTo("Host", listOf("car" to B.hidden(TypeExp.TypeId("door")))),
                B.yieldTo("Guest", listOf("choice" to TypeExp.TypeId("door"))),
                B.yieldTo(
                    "Host",
                    listOf("goat" to TypeExp.TypeId("door")),
                    where = (B.m("Host", "goat") neq B.m("Guest", "choice")) and
                            (B.m("Host", "goat") neq B.m("Host", "car"))
                ),
                B.yieldTo("Guest", listOf("switch" to BOOL)),
                B.reveal("Host", listOf("car" to TypeExp.TypeId("door"))),
                value = Value(
                    mapOf(
                        "Guest" to B.ite(
        ...
        "Prisoner's Dilemma type checking" {
            val p = B.program(
                ...
                B.yieldTo("Alice", listOf("cooperate" to BOOL)),
                B.yieldTo("Bob", listOf("cooperate" to BOOL)),
                value = Value(
                    mapOf(
                        "Alice" to B.ite(
        ...
        "Multi-player lottery with many players" {
            val types = mapOf(
                "choice" to TypeExp.Subset(setOf(B.n(1), B.n(2), B.n(3), B.n(4), B.n(5)))
            )
            val p = B.program(
                types,
                ...
                B.yieldTo("P1", listOf("c" to TypeExp.TypeId("choice"))),
                B.yieldTo("P2", listOf("c" to TypeExp.TypeId("choice"))),
                B.yieldTo("P3", listOf("c" to TypeExp.TypeId("choice"))),
                value = Value(
                    mapOf(
                        "P1" to B.ite(
        ...
        "should provide helpful error messages" - {
            ...
            withData(
                ...
                ErrorMessageCase(
                    ...
                    listOf("UnknownRole", "undefined")
                ),
                ErrorMessageCase(
                    B.program(
                        B.join("P"),
                        B.yieldTo("P", listOf(B.bl("x"))),
                        B.reveal("P", listOf("x" to INT)) // must be hidden
                    ),
