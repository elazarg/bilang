...
\begin{grammar}
    <prog> ::= done
         \alt receive (i, tx) <case list> end   % external choice
         \alt par <prog> and <prog> end
    
	<cmd> ::= yield ev; <prog>                 % yield + external choice
...
\judge{&Receive}{
	...
	\semfunc[\text{match (i, tx) Cs end}, (i', tx')] = \semfunc[\text{Cs[i/i', tx/tx']}]  % receiving TX
}\\
\\
\judge{&Yield}{
	-
}{
	\semfunc[\text{yield ev; p}] = (p, ev)  % waiting for TX
}\\
\\
