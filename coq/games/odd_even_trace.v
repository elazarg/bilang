...
Definition schedule := forall st_e st_o p,
  {p': prefix | owner p E /\ p' = st_e p} + {p': prefix | owner p O /\ p' = st_o p}.
...
Definition strategy_preserved (r1 r2: role) := exists to_high to_low,
  ...
  wins st1_h (to_high st2_l) -> forall (s: schedule), High.wins sched (to_low st1_h) st2_l.
...
This can be easily (?) described using extensive form, where information sets define dependence,
(e.g. and the scheduler is the one who deliver messages.

Q: why then games should be defined differently in the recent papers?
*)
